...
function parseDividendData(rawData: any[], latestPrice: number) {
  ...
  for (const item of rawData) {
    ...
    if (date >= oneYearAgo) ttmDividends += dividend;
    if (date.getFullYear() === currentYear - 1) lfyDividends += dividend;
  }
  ...
  if (lfyDividends === 0) {
    for (const item of rawData) {
      ...
      const date = new Date(dateStr);
      if (isNaN(date.getTime())) continue;
      if (date.getFullYear() === currentYear - 2) lfyDividends += amount / 10;
    }
  }
...
async function processStockWithRetry(stock: any, retries = 1): Promise<StockItem | null> {
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (!code || !name || marketCap <= 0 || latestPrice <= 0) return null;
...
async function processBatch(stocks: any[]): Promise<StockItem[]> {
  const results: StockItem[] = [];
  
  for (let i = 0; i < stocks.length; i += CONCURRENCY_LIMIT) {
    const batch = stocks.slice(i, i + CONCURRENCY_LIMIT);
    const batchResults = await Promise.all(batch.map(processStockWithRetry));
    results.push(...batchResults.filter((r): r is StockItem => r !== null));
